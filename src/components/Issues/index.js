...
class Issues extends React.PureComponent {
  ...
  updateIssue(item, info) {
    ...
    if (selected && item.owner === selected.owner && item.repo === selected.repo && item.number === selected.number) {
    ...
    return (
      <React.Fragment>
        <List
          ...
          pagination={false}
          renderItem={item => (
            <List.Item onClick={e => this.showIssueDetail(e, item)}>
              ...
              <div className={item.read ? styles.title : styles.unreadTitle}>
                <a
                  ...
                  href={`https://github.com/${item.owner}/${item.repo}/issues/${item.number}`}
                >
                  {item.title}
                </a>
              </div>
